...
/**
 * Action que desindexa un microsite
 * <P>
 *
...
public class DesindexaAction extends BaseAction {
	...
	public ActionForward doExecute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request,
		...
		if (microsite.getId() != null) {
			MicroLog.addLog("Inici DesIndexació Microsite: [" + microsite.getId() + "] , Usuari: ["
					+ request.getSession().getAttribute("username") + "]");
			final Long site = new Long("" + microsite.getId());

			// indexo.desindexarMicrosite(site);

			MicroLog.addLog("Fi DesIndexació Microsite: [" + microsite.getId() + "] , Usuari: ["
					+ request.getSession().getAttribute("username") + "]");
