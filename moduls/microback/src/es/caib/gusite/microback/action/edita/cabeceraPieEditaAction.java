...
public class cabeceraPieEditaAction extends BaseAction {
	...
	public ActionForward doExecute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request,
		...
		if (("" + request.getParameter("accion")).equals("cabpie")) {
			...
			final Microsite micrositeBean = guardar(request, microForm);
			...
			final Microsite micrositeBean = traducir(request, microForm, "CABPIE");
	...
	private Microsite traducir(final HttpServletRequest request, final microForm microForm, final String configuracion)
		...
		final Microsite micrositeBean = micrositeDelegate.obtenerMicrosite((Long) microForm.get("id"));
	 ...
	 * @throws Exception
	 */
	private Microsite guardar(final HttpServletRequest request, final microForm microForm) throws Exception {

		final MicrositeDelegate micrositeDelegate = DelegateUtil.getMicrositeDelegate();
		final Microsite micrositeBean = setFormtoBean(microForm);

		final Date fecha = new Date();
		final String[] fechaHora = {
				DateFormat.getDateInstance(DateFormat.LONG, new Locale("ca", "ES")).format(fecha).replaceAll("/", "de"),
				DateFormat.getTimeInstance(DateFormat.LONG, new Locale("ca", "ES")).format(fecha).replaceAll("/",
						"de") };

		// Guardamos cambios en BBDD
		micrositeDelegate.grabarMicrosite(micrositeBean);

		log.info("Configuración Microsite modificada - Id: " + microForm.get("id"));

		micrositeBean.setMvsCssTiny(tagCSS(micrositeBean.getEstiloCSS()));

		micrositeBean.setMensajeInfo(getResources(request).getMessage("micro.modifcabpie")
	 ...
	 * @throws Exception
	 */
	private Microsite setFormtoBean(final microForm microForm) throws Exception {
		...
		final List<Idioma> langs = DelegateUtil.getIdiomaDelegate().listarIdiomas();
		for (int i = 0; i < langs.size(); i++) {
			final Idioma lang = langs.get(i);
			TraduccionMicrosite traduccion = (TraduccionMicrosite) micrositeBean
					.getTraduccion(lang.getCodigoEstandar());
	 ...
	 * @throws Exception
	 */
	private void cargar(final HttpServletRequest request, final microForm microForm) throws Exception {
