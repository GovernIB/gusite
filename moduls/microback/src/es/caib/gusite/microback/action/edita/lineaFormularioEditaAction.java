...
public class lineaFormularioEditaAction extends BaseAction {
	...
	public ActionForward doExecute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request,
		...
		final lineaFormularioForm lineaFormularioForm = (lineaFormularioForm) form;
		...
		if (("" + request.getParameter("accion")).equals(getResources(request).getMessage("operacion.crear"))) {
			...
			final Long idContacto = new Long(("" + request.getParameter("idcontacto")));
			setBeantoForm(lineaFormularioForm, idContacto);
			return mapping.findForward("detalle");
			/********************** GUARDAR *************************/
		} else if (("" + request.getParameter("accion"))
				.equals(getResources(request).getMessage("operacion.guardar"))) {

			final Lineadatocontacto lineaDatoContacto = setFormtoBean(lineaFormularioForm);
			...
			if (lineaFormularioForm.get("id") == null) {
				...
				addMessageWithDate(request, "formu.linea.mensa.modifagenda");
			...
			final Long idLinea = (Long) lineaFormularioForm.get("id");
			final Long idContacto = (Long) lineaFormularioForm.get("idcontacto");

			final ContactoDelegate bdFormu = DelegateUtil.getContactoDelegate();
	...
	/**
	 * Método que vuelca el formulario de una linea de Contacto en el Bean
	 * LineaDatoContacto
	 * 
	 * @author Indra
	 * @param lineaFormularioForm
	 *            formulario dinámico enviado por usuario
	 * @return LineDatoContacto Bean de tipo LineaDatoContacto
	 * @throws Exception
	 */
	private Lineadatocontacto setFormtoBean(final lineaFormularioForm lineaFormularioForm) throws Exception {
		...
		if (lineaFormularioForm.get("id") == null)
			...
			lineaDatoContacto = contactoDelegate.obtenerLinea((Long) lineaFormularioForm.get("id"));
		...
		final String[] etiquetas = (String[]) lineaFormularioForm.get("etiq");
		final String[] textos = (String[]) lineaFormularioForm.get("textos");
		...
		final String[] etiquetasTipo4o5 = new String[etiquetas.length];

		if (lineaDatoContacto.getTipo().equals("4") || lineaDatoContacto.getTipo().equals("5")) {

			String fila;
			int limite = 0;
			if (lineaDatoContacto.getTipo().equals("4"))
				limite = Integer.parseInt("" + lineaFormularioForm.get("numSelectors"));
		...
		Map<String, TraduccionLineadatocontacto> mapaTraducciones = new HashMap<String, TraduccionLineadatocontacto>(
				langs.size());
		TraduccionLineadatocontacto traduccionLineaDatoContacto;

		for (int i = 0; i < langs.size(); i++) {
			final Idioma lang = (Idioma) langs.get(i);
			...
			if (lineaFormularioForm.get("id") == null) {
				...
				if (etiquetasTipo4o5[i] != null)
					traduccionLineaDatoContacto.setTexto(etiquetasTipo4o5[i]);
				else
					traduccionLineaDatoContacto.setTexto(etiquetas[i]);
	 ...
	 * @throws Exception
	 */
	private void setBeantoForm(final lineaFormularioForm lineaFormularioForm, final Long idLineaContacto)
			throws Exception {

		final ContactoDelegate contactoDelegate = DelegateUtil.getContactoDelegate();
		final Lineadatocontacto lineaDatoContactoBean = contactoDelegate.obtenerLinea(idLineaContacto);
		...
		final IdiomaDelegate delegate = DelegateUtil.getIdiomaDelegate();
		final List<?> idis = delegate.listarLenguajes();
		final int numLangs = idis.size();
		final String[] etis = new String[numLangs];
		final String[] textos = new String[numLangs * Lineadatocontacto.NUMERO_MAXIMO_SELECTOR];
		int max_lin = -1000;
		String trad_texto = "";
		Integer resultVectorlenght = new Integer(0);
		for (int i = 0; i < numLangs; i++) {
			if (lineaDatoContactoBean.getTraduccion("" + idis.get(i)) != null) {
				if (((TraduccionLineadatocontacto) lineaDatoContactoBean.getTraduccion("" + idis.get(i)))
						.getTexto() != null)
					trad_texto = ((TraduccionLineadatocontacto) lineaDatoContactoBean.getTraduccion("" + idis.get(i)))
							.getTexto().toString();
				else
					trad_texto = "";

				final String[] vector = trad_texto.split("#");

				if (lineaDatoContactoBean.getTipo().equals("4") || lineaDatoContactoBean.getTipo().equals("5")) {
					for (int m = 0; m < vector.length; m++) {
						if (m == 0)
							etis[i] = vector[0].replace("&num;", "#"); // El primero es la etiqueta
						if (m > 0)
							textos[(numLangs * (m - 1)) + i] = vector[m].replace("&num;", "#");
					}
					final int cont = Cadenas.countSubstr(trad_texto, "#");
					if (cont > 0) {
				...
				if (vector.length > max_lin)
					max_lin = vector.length;
			}
		}
		if (lineaDatoContactoBean.getTipo().equals("4"))
			lineaFormularioForm.set("numSelectors", new Integer(max_lin - 1));
		if (lineaDatoContactoBean.getTipo().equals("5"))
			lineaFormularioForm.set("numSelectors2", new Integer(max_lin - 1));
	 ...
	 * @throws Exception
	 */
	private void traducir(final HttpServletRequest request, final lineaFormularioForm lineaFormularioForm)
			throws Exception {
		...
		final TraduccionLineadatocontacto lineadatocontactoOrigen = new TraduccionLineadatocontacto();
		...
		if (lineaFormularioForm.get("tipo").equals("4") || lineaFormularioForm.get("tipo").equals("5")) {
			...
			boolSelector = true;
			if (lineaFormularioForm.get("tipo").equals("4"))
				limiteSelector = Integer.parseInt("" + lineaFormularioForm.get("numSelectors"));
			if (lineaFormularioForm.get("tipo").equals("5"))
				limiteSelector = Integer.parseInt("" + lineaFormularioForm.get("numSelectors2"));
		...
		final String[] etiq = new String[traductor.getListLang().size()];
		int indexIdioma = 0;
		etiq[0] = etiquetas[0];
		...
		while (itLang.hasNext()) {
			...
			final TraduccionLineadatocontacto lineadatocontactoDesti = new TraduccionLineadatocontacto();
			...
			if (idiomasMicro.contains(idiomaDesti)) {
				...
				if (!idiomaOrigen.equals(idiomaDesti)) {
					...
					if (traductor.traducir(lineadatocontactoOrigen, lineadatocontactoDesti, boolSelector,
							limiteSelector, numLangs, indexIdioma, textoSelector)) {
		...
		log.info("Traducción Linea Formulario - Id: " + lineaFormularioForm.get("id"));
	}
