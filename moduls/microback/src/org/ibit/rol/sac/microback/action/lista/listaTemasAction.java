...
/**
 * Action que valida y trata el listado de temas de un microsite <BR>
 * <P>
 * 	Definición Struts:<BR>
 *  action path="/temasAcc" <BR> 
 *  name="listaActionForm" <BR> 
 *  scope="request" <BR>
 *  forward name="detalleTema" path="/detalleTema.jsp" 
 *  
 *  @author Indra
 */
public class listaTemasAction extends BaseAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception  {
        ...
        //************* ERROR DE VALIDACION **********************
        //********************************************************
        if (request.getSession().getAttribute("temaForm")!=null && request.getAttribute(Globals.ERROR_KEY)!=null) {
        	TraDynaActionForm fdet=(TraDynaActionForm) request.getSession().getAttribute("temaForm");
        	request.setAttribute("temaForm", fdet);
            return mapping.findForward("detalleTema");
        }
        //********************************************************
        //********************** CREAMOS *************************
        //********************************************************
        if ( f.getAccion().equals("crear")) {
        	request.getSession().removeAttribute("temaForm");
        	return mapping.findForward("detalleTema");
        }
        //********************************************************
        //********************* BORRAMOS *************************
        ...
        if ( f.getAccion().equals("borrar")) {
            ...
            for (int i=0;i<f.getSeleccionados().length;i++) {
                ...
                bdTema.borrarTema(id);
                lis+=id+", ";
            }
            lis=lis.substring(0,lis.length()-2);
            
            addMessage(request, "mensa.listatemas");
            addMessage(request, "mensa.listatemborradas", new String(lis));
