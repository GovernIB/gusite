...
function iniciarMenu() {
	...
	for(i=0;i<buttons.length;i++) {
		buttons[i].onclick = function() {
			...
			document.getElementById('padreCM').value = idMenu;
			mostrarFondo();
	...
	for(i=0;i<buttons.length;i++) {
		buttons[i].onclick = function() {
			...
			if(hijosNum != 0) {
				alert('No pots esborrar un node amb fills!');
				return false;
			}
			//
			confirmar = confirm('Est? segur de voler esborrar aquest item?');
			if(confirmar) {
				parametros = "idMenu=" + idMenu;
				//enviaPeticion('post', 'index_borrarNodo.php', parametros, true, resBorrarNodo); // peticion AJAX
				return idMenu;
			} else {
				return false;
			}
...
function cambiarPosicion(obj) {
	...
	//document.getElementById('ventanaUser').value = usuariY; // solo PRUEBAS!!
	// y a?adimos eventos
...
function cambiarPosicionGo(event) {
	...
	if(ratonY >= usuariY - 40) document.documentElement.scrollTop = document.documentElement.scrollTop + 5;
	if(ratonY >= usuariY - 20) document.documentElement.scrollTop = document.documentElement.scrollTop + 10;
	...
	//document.getElementById('CapaXX').value = idCapaOver; // solo PRUEBAS!!
	return idCapaOver;
	if(document.all) {
		...
		if(document.getElementById(capaOverID) != null) {
			...
			if(imgALT.indexOf('Obrir') != -1) {
				...
				if(clase == 'nivel2') {
					...
					else document.getElementById('X').className = 'pagCnoVisibleNivel2';
				}
			}
		}
...
function ordenarNodes() {
	divs = document.getElementById('menuArbol').getElementsByTagName('div');
	for(i=0;i<divs.length;i++) {
		if(divs[i].id != 'nuevo') divs[i].getElementsByTagName('input')[1].value = i + 1;
	}
}
