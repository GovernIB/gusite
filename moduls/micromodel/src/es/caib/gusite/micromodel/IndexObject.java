...
public class IndexObject extends ModelIndexObject {
	...
	public void addArchivo(Archivo archi) {
		if (archi != null && archi.getPeso() > 0) {
			Extractor extractor = ExtractorFactory
					.getExtractor(archi.getMime());
			if (extractor != null) {
				try {
					String aText = extractor
							.extractText(new ByteArrayInputStream(archi
									.getDatos()));
					this.addTextLine(aText);
				} catch (IOException e) {
					;
				}
			}
	...
	public void addDescripcion(Archivo archivo) {
		if (archivo != null && archivo.getPeso() > 0) {
			Extractor extractor = ExtractorFactory.getExtractor(archivo
					.getMime());
			if (extractor != null) {
				try {
					String aText = extractor
							.extractText(new ByteArrayInputStream(archivo
									.getDatos()));
					if (aText.length() > 200) {
						this.addDescripcionLine(aText.substring(0, 199) + "...");
					} else {
						this.addDescripcionLine(aText);
					}
				} catch (IOException e) {
					;
				}
