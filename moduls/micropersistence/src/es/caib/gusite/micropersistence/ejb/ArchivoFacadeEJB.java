...
public abstract class ArchivoFacadeEJB extends HibernateEJB {
	...
	public Archivo obtenerArchivo(Long id) {
		...
		try {
			...
			if (ArchivoUtil.almacenarEnFilesystem()) {
				obtenerDatosArchivoExportado(archi);
			}
     ...
     * (necesario para proceso de exportación a disco).
     * 
     * @ejb.interface-method
     * @ejb.permission unchecked="true"
     */
	public List<Object[]> obtenerTodosLosArchivosSinBlobs() {
		...
		try {
			...
			// amartin: order by a.idmicrosite desc, a.id asc para que el último microsite sea el 0 (¿archivos comunes?).
	...
	public Archivo obtenerArchivobyName(Long site, String nombre) {
		...
		Session session = getSession();
		try {
			// TODO amartin: ¿usar método checkSite() aquí, propio de este mismo EJB, para ahorrar código?
			...
			while (iterArchi.hasNext()) {
				...
				archi = (Archivo)iterArchi.next();
				String hql = "from Noticia noti join noti.traducciones trad join trad.docu doc where doc.id='" + archi.getId() + "' and noti.idmicrosite='" + site + "'";
	...
	public Long grabarArchivo(Archivo a) {
		...
		try {
			...
			if (ArchivoUtil.almacenarEnFilesystem()) {
				...
				if (!nuevo) {
					Archivo aOld = obtenerArchivo(a.getId());
	...
	public void indexBorraArchivo(Long id) {
		...
		try {
			...
			for (int i = 0; i < langs.size(); i++) {
				indexerDelegate.borrarObjeto(Catalogo.SRVC_MICRO_DOCUMENTOS + "." + id, "" + langs.get(i));
			}
	...
	private void obtenerDatosArchivoExportado(Archivo a) throws IOException {
		...
		if (ArchivoUtil.existeArchivoEnFilesystem(a)) {
			...
			a.setDatos(ArchivoUtil.obtenerDatosArchivoEnFilesystem(a));
			 ...
			 // TODO amartin: consultar si quieren que se exporte el archivo a disco en este caso y luego se
