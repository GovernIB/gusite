 ...
 * 
 * @ejb.bean name="sac/micropersistence/IndexerFacade"
...
public abstract class IndexerFacadeEJB extends HibernateEJB {
		...
		try {
			if (indexObject != null) {
				if (indexObject.getTitulo() != null
						&& indexObject.getTitulo().length() > 0) {
					this.addDocumentoAlWriter(writer, indexObject, idi);
				}
			}
	...
	public void borrarObjeto(String id, String idi) {
		...
		try {
			Directory directory = this.getHibernateDirectory(idi);
	...
	public void indexarObjeto(Object objeto) throws DelegateException {
		...
		if (this.isBloqueado()) {
			...
			if (objeto instanceof Encuesta) {
				EncuestaDelegate bdEncuesta = DelegateUtil
						.getEncuestaDelegate();
			...
			if (objeto instanceof Contenido) {
				ContenidoDelegate bdContenido = DelegateUtil
	...
	public void optimizar(List<String> langs) throws IOException {
		...
		for (String lang : langs) {
			Directory directory = this.getHibernateDirectory(lang);
			IndexWriter writer = new IndexWriter(directory,
					Analizador.getAnalizador(lang), MaxFieldLength.UNLIMITED);
			try {
				writer.setMergeFactor(20);
				writer.setMaxMergeDocs(Integer.MAX_VALUE);
				this.optimizarIndice(writer);
			} finally {
				writer.close();
				directory.close();
			}
	...
	public void confeccionaDiccionario(String idioma) {
		...
		this.actualizarDiccionario(idioma);
	}
	...
	public void desindexarMicrosite(Long idsite) {
		...
		try {
			...
			for (int i = 0; i < langs.size(); i++) {
				Directory directory = this.getHibernateDirectory(""
						+ langs.get(i));
				IndexReader reader = IndexReader.open(directory, false);
				try {
					reader.deleteDocuments(new Term(Catalogo.DESCRIPTOR_MICRO,
							"" + idsite.longValue()));
				} finally {
					try {
						reader.close();
	...
	/**
	 * Busca documentos en un microsite para un idioma concreto, con opción de
	 * sugerir en caso de haber encontrado nada interesante.
	 * 
	 * @ejb.interface-method
	 * @ejb.permission unchecked="true"
	 */
	public IndexResultados buscar(String micro, String idioma, String idlista,
			String cadena, boolean sugerir) {

		long startTime = System.currentTimeMillis();
		try {
			idioma = idioma.toLowerCase();
			Directory directory = this.getHibernateDirectory(idioma);
			...
			if (cadena != null && cadena.length() > 0) {
				...
				Vector<String> words = this.getWords(cadena, CAMPO_BUSQUEDAS,
						idioma);
				...
				if (words.size() > 1) {
					...
					for (int i = 0; i < words.size(); i++) {
						((PhraseQuery) query_compuestaAND).add(new Term(
								CAMPO_BUSQUEDAS, words.elementAt(i)));
						((PhraseQuery) query_compuestaOR).add(new Term(
								CAMPO_BUSQUEDAS, words.elementAt(i)));
						expandedQueries[i] = new TermQuery(new Term(
								CAMPO_BUSQUEDAS, words.elementAt(i)));
					}
					query_compuestaOR = query_compuestaOR
							.combine(expandedQueries); // combinarlas entre si.
														// Es equivalente a un
														// OR de todo con todo

				} else if (words.size() == 1) {
					query_compuestaAND = new TermQuery(new Term(
							CAMPO_BUSQUEDAS, words.elementAt(0)));
					query_compuestaOR = new TermQuery(new Term(CAMPO_BUSQUEDAS,
							words.elementAt(0)));
			...
			if (micro != null && micro.length() > 0) {
				queryAND.add(new TermQuery(new Term(Catalogo.DESCRIPTOR_MICRO,
						micro)), BooleanClause.Occur.MUST);
				queryOR.add(new TermQuery(new Term(Catalogo.DESCRIPTOR_MICRO,
						micro)), BooleanClause.Occur.MUST);
			}

			if (idlista != null && idlista.length() > 0) {
				queryAND.add(new TermQuery(new Term(
						Catalogo.DESCRIPTOR_CLASIFICACION,
						Catalogo.SRVC_MICRO_ELEMENTOS + "." + idlista)),
						BooleanClause.Occur.MUST);
				queryOR.add(new TermQuery(new Term(
						Catalogo.DESCRIPTOR_CLASIFICACION,
						Catalogo.SRVC_MICRO_ELEMENTOS + "." + idlista)),
						BooleanClause.Occur.MUST);
			}

			// Evitamos los documentos caducados o que no han sido publicados
			// aun
			String hoy = new java.text.SimpleDateFormat("yyyyMMdd")
					.format(new Date());

			TermRangeQuery publica = new TermRangeQuery(
					Catalogo.DESCRIPTOR_PUBLICACION, "00000000", hoy, true,
					true);
			queryAND.add(publica, BooleanClause.Occur.MUST);
			queryOR.add(publica, BooleanClause.Occur.MUST);

			TermRangeQuery caduca = new TermRangeQuery(
					Catalogo.DESCRIPTOR_CADUCIDAD, hoy, "99999999", true, true);
			...
			// AVISO: Note this query can be slow, as it needs to iterate over
			// many terms
			// query.add( new WildcardQuery(new Term ("id",
			// "MICRO.CONTENIDOS.*")) , BooleanClause.Occur.MUST);
			...
			if ((hits.totalHits < MIN_HITS || hits.scoreDocs[0].score < MIN_SCORE)
					&& sugerir) {
				String cadenaSugerida = "";
				Query quisoDecir = this.sugerir(cadena, idioma);

				if (quisoDecir != null) {
					cadenaSugerida = quisoDecir.toString(CAMPO_BUSQUEDAS)
							.replace('+', ' ');
				}

				TopDocs hits2 = searcher.search(queryOR, RESULTATS_CERCA_TOTS);
				endTime = System.currentTimeMillis();
				res = new IndexResultados(this.extractHits(hits2, cadena,
						directory), hits2.totalHits, (endTime - startTime),
						cadena, cadenaSugerida, "1");
				// res= new IndexResultados(null, 0, (endTime - startTime),
				// cadena, cadenaSugerida, null );

			} else {
				endTime = System.currentTimeMillis();
				res = new IndexResultados(this.extractHits(hits, cadena,
						directory), hits.totalHits, (endTime - startTime),
						cadena, null, null);
	...
	public ArrayList<String> diccionario(String idi) {
		...
		try {
			IndexReader indexReader = IndexReader.open(this
					.getHibernateDirectory(idi));
			Dictionary dictionary = new LuceneDictionary(indexReader,
					CAMPO_BUSQUEDAS);
			SpellChecker spellChecker = new SpellChecker(
					this.getHibernateDirectory(idi + "/dicc"));
			// spellChecker.clearIndex();
			IndexWriterConfig conf = new IndexWriterConfig(
					Constants.LUCENE_VERSION, Analizador.getAnalizador(idi));
			try {
	...
	public void reindexarMicrosite(Long idsite) {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				while (itercont.hasNext()) {
					...
					if (con.getFcaducidad() == null
							|| con.getFcaducidad().getTime() >= hoy.getTime()) {
			...
			stlog = new StringBuffer("");
			hql = "from Agenda age where age.idmicrosite = "
					+ idsite.toString();
			...
			stlog = new StringBuffer("");
			hql = "from Encuesta encu where encu.idmicrosite = "
					+ idsite.toString();
			...
			stlog = new StringBuffer("");
			hql = "from Noticia noti where noti.idmicrosite = "
					+ idsite.toString();
			...
			stlog = new StringBuffer("");
			query = session
					.createQuery("from Archivo archi where archi.idmicrosite = "
							+ idsite.toString() + " and archi.pagina is null");
			list = query.list();
			iter = list.iterator();
			...
			log.info("index documentos [site:" + idsite + "] "
					+ stlog.toString());
			session.flush();
			session.clear();

			// Optimizamos el indice y diccionario
			this.optimizar(langs);
			for (int i = 0; i < langs.size(); i++) {
				this.confeccionaDiccionario("" + langs.get(i));
			}
	...
	public Directory getHibernateDirectory(String idi) {
		...
		try {
			...
			if (!IndexReader.indexExists(directory)) {
				new IndexWriter(directory, this.analyzer, true,
	...
	private synchronized void addDocumentoAlWriter(IndexWriter writer,
			IndexObject indexObject, String idioma) {

		// Añade un documento
		try {
			Document document = this.getDocument(indexObject);
			writer.addDocument(document, Analizador.getAnalizador(idioma));

		} catch (IOException e) {
			log.error("addDocumentoAlWriter [" + OPER_ADDDOCUMENT
					+ "] de microsites: " + e.getMessage(), e);
		}
	}
		...
		try {
			...
			try {
				Dictionary dictionary = new LuceneDictionary(indexReader,
						CAMPO_BUSQUEDAS);
				SpellChecker spellChecker = new SpellChecker(
						this.getHibernateDirectory(idioma + "/dicc"));
				IndexWriterConfig indexWriterConfig = new IndexWriterConfig(
						Constants.LUCENE_VERSION,
						Analizador.getAnalizador(idioma));
				spellChecker.indexDictionary(dictionary, indexWriterConfig,
						true);
			} finally {
				indexReader.close();
			}
			...
			log.error("actualizarDiccionario [" + OPER_DICCIONARIO
					+ "] de microsites: " + e.getMessage(), e);
		}
	}
	...
	private synchronized void optimizarIndice(IndexWriter writer) {
		...
		try {
			...
			log.error("operarConWriter [" + OPER_OPTIMIZAR
					+ "] de microsites: " + e.getMessage(), e);
		}
	}
	...
	private Document getDocument(IndexObject indexObject) {
		...
		if (indexObject.getId() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_ID, indexObject.getId(),
					Field.Store.YES, Field.Index.toIndex(true, false)));
		}

		if (indexObject.getClasificacion() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_CLASIFICACION,
					indexObject.getClasificacion(), Field.Store.YES,
					Field.Index.toIndex(true, false)));
		}

		/* Descriptores filtro */
		if (indexObject.getMicro() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_MICRO, indexObject
					.getMicro().toString(), Field.Store.YES, Field.Index
					.toIndex(true, false)));
		}

		if (indexObject.getRestringido() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_RESTRINGIDO, indexObject
					.getRestringido(), Field.Store.YES, Field.Index.toIndex(
					true, false)));
		}

		if (indexObject.getFamilia() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_FAMILIA, indexObject
					.getFamilia().toString(), Field.Store.YES, Field.Index
					.toIndex(true, false)));
		}

		if (indexObject.getSeccion() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_SECCION, indexObject
					.getSeccion(), Field.Store.YES, Field.Index.toIndex(true,
					false)));
		}

		if (indexObject.getMateria() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_MATERIA, indexObject
					.getMateria(), Field.Store.YES, Field.Index.toIndex(true,
					false)));
		}

		if (indexObject.getUo() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_UO, indexObject.getUo(),
					Field.Store.YES, Field.Index.toIndex(true, false)));
		}

		/* descriptores informacion del documento */
		if (indexObject.getTitulo() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_TITULO, indexObject
					.getTitulo(), Field.Store.YES, Field.Index.toIndex(true,
					false)));
		}

		if (indexObject.getTituloserviciomain() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_TITULO_SRV_MAIN,
					indexObject.getTituloserviciomain(), Field.Store.YES,
					Field.Index.toIndex(true, false)));
		}

		if (indexObject.getText() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_TEXT, indexObject
					.getText(), Field.Store.YES, Field.Index
					.toIndex(true, true)));
		}

		if (indexObject.getTextopcional() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_TEXTOPCIONAL,
					indexObject.getTextopcional(), Field.Store.YES, Field.Index
							.toIndex(true, true)));
		}

		if (indexObject.getUrl() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_URL, indexObject
					.getUrl(), Field.Store.YES, Field.Index.NO));
		}

		if (!indexObject.getPublicacion().equals("")) {
			document.add(new Field(Catalogo.DESCRIPTOR_PUBLICACION, indexObject
					.getPublicacion(), Field.Store.YES, Field.Index.toIndex(
					true, false)));
		} else {
			document.add(new Field(Catalogo.DESCRIPTOR_PUBLICACION, "00000000",
					Field.Store.YES, Field.Index.toIndex(true, false)));
		}

		if (!indexObject.getCaducidad().equals("")) {
			document.add(new Field(Catalogo.DESCRIPTOR_CADUCIDAD, indexObject
					.getCaducidad(), Field.Store.YES, Field.Index.toIndex(true,
					false)));
		} else {
			document.add(new Field(Catalogo.DESCRIPTOR_CADUCIDAD, "99999999",
					Field.Store.YES, Field.Index.toIndex(true, false)));
		}

		if (indexObject.getDescripcion() != null) {
			document.add(new Field(Catalogo.DESCRIPTOR_DESCRIPCION, indexObject
					.getDescripcion(), Field.Store.YES, Field.Index.toIndex(
					true, false)));
		}
	...
	private Vector getWords(String cadena, String campo, String idioma) {
		...
		TokenStream tokens = Analizador.getAnalizador(idioma).tokenStream(
				campo, new StringReader(cadena));
	...
	private List<IndexEncontrado> extractHits(TopDocs hits, String cadena,
		...
		for (ScoreDoc scoreDoc : hits.scoreDocs) {
			...
			hitList.add(new IndexEncontrado(doc.get("id"), doc
					.get(Catalogo.DESCRIPTOR_TITULO), doc
					.get(Catalogo.DESCRIPTOR_DESCRIPCION), doc
					.get(Catalogo.DESCRIPTOR_MICRO), doc
					.get(Catalogo.DESCRIPTOR_URL), new Float(
					100 * scoreDoc.score).intValue()));
		}
	...
	private Query sugerir(String queryString, String idi) throws ParseException {
		...
		QuerySuggester querySuggester = new QuerySuggester(CAMPO_BUSQUEDAS,
				Analizador.getAnalizador(idi), this.getHibernateDirectory(idi
						+ "/dicc"), idi);
		querySuggester.setDefaultOperator(QueryParser.AND_OPERATOR);
		Query query = querySuggester
				.parse((queryString.length() > 0) ? queryString : " ");

		return querySuggester.hasSuggestedQuery() ? query : null;
	}

	/**
	 * Clase que sobreescribe un método de QueryParser para permitir hacer
	 * sugerencias cuando las búsquedas sean compuestas
	 */
	public class QuerySuggester extends QueryParser {

		private boolean suggestedQuery = false;

		public QuerySuggester(String field, Analyzer analyzer, Directory dir,
				String idi) {
