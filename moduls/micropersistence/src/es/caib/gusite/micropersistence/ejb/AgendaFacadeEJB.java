...
public abstract class AgendaFacadeEJB extends HibernateEJB {
	...
	public void init(final Long site) {
		...
		super.where = " where trad.id.codigoIdioma = '" + Idioma.getIdiomaPorDefecto() + "' and agenda.idmicrosite="
				+ site.toString();
		super.whereini = " ";
		super.orderby = "";
	...
	public void init(final Long site, final String idiomapasado) {
		...
		super.from = " from Agenda agenda join agenda.traducciones trad ";
		super.where = " where (trad.id.codigoIdioma='" + Idioma.getIdiomaPorDefecto() + "' or trad.id.codigoIdioma='"
				+ idiomapasado + "')  and agenda.idmicrosite=" + site.toString();
		super.whereini = " ";
		super.orderby = "";
	...
	public Long grabarAgenda(final Agenda agenda) throws DelegateException {
		...
		try {
			...
			final Map<String, TraduccionAgenda> listaTraducciones = new HashMap<String, TraduccionAgenda>();

			if (nuevo) {

				final Iterator<TraduccionAgenda> it = agenda.getTraducciones().values().iterator();
				while (it.hasNext()) {
					final TraduccionAgenda trd = it.next();
				...
				for (final TraduccionAgenda trad : agenda.getTraducciones().values()) {

					final TraduccionAgenda tradOriginal = (TraduccionAgenda) agendaOriginal
					...
					if (trad.getDocumento() != null) {
						if (trad.getDocumento().getId() == null) { // Condición de nuevo documento.
							...
							pendienteDel.grabarSolrPendiente(EnumCategoria.GUSITE_AGENDA.toString(), agenda.getId(),
									trad.getDocumento().getId(), IndexacionUtil.REINDEXAR);
						} else if (trad.getDocumento().getDatos() != null) { // Condición de actualizar documento.
						...
						if (tradOriginal.getDocumento() != null) { // Condición de borrado de documento.
							...
							final SolrPendienteDelegate pendienteDel = DelegateUtil.getSolrPendienteDelegate();
							pendienteDel.grabarSolrPendiente(EnumCategoria.GUSITE_AGENDA.toString(), agenda.getId(),
									tradOriginal.getDocumento().getId(), IndexacionUtil.DESINDEXAR);
						}
					}
					...
					if (trad.getImagen() != null) {
						...
						else if (trad.getImagen().getDatos() != null) // Condición de actualizar documento.
			...
			if (nuevo) {
				...
				for (final TraduccionAgenda trad : listaTraducciones.values()) {
			...
			// Indexamos
			final SolrPendienteDelegate pendienteDel = DelegateUtil.getSolrPendienteDelegate();
			pendienteDel.grabarSolrPendiente(EnumCategoria.GUSITE_AGENDA.toString(), agenda.getId(), null,
	 ...
	 * @ejb.permission unchecked="true"
	 */
	public Agenda obtenerAgenda(final Long id) {
		final Session session = this.getSession();
		try {
			final Agenda agenda = (Agenda) session.get(Agenda.class, id);
			return agenda;
		} catch (final HibernateException he) {
			throw new EJBException(he);
		} finally {
	 ...
	 * @ejb.permission unchecked="true"
	 */
	public List<Agenda> listarAgendasrec(final String idiomapasado) {
		final Session session = this.getSession();
		try {
			this.parametrosCons(); // Establecemos los parámetros de la
									// paginación
			final Query query = session.createQuery(this.select + this.from + this.where + this.orderby);
			final ArrayList<Agenda> lista = new ArrayList<Agenda>();
			...
			while (this.tampagina > i++) {
				...
				final Agenda age = new Agenda();
				age.setIdmicrosite((Long) fila[3]);
				age.setId((Long) fila[0]);
				age.setFinicio((Date) fila[2]);
				final TraduccionAgenda tradage = new TraduccionAgenda();
	 ...
	 * @ejb.permission unchecked="true"
	 */
	public ArrayList<Agenda> listarAgendas(final String idioma) {
		final Session session = this.getSession();
		try {
			this.parametrosCons(); // Establecemos los parámetros de la
									// paginación
	 ...
	 * @ejb.permission unchecked="true"
	 */
	public ArrayList<?> listarAgendas(final Date fecha, final String idioma) {
		final Session session = this.getSession();
		try {
			// parametrosCons(); // Establecemos los parámetros de la paginación
			final java.sql.Date dt = new java.sql.Date(fecha.getTime());
	...
	public void borrarAgenda(final Long id, final boolean indexar) throws DelegateException {
		...
		try {
			...
			final List<TraduccionAgenda> traducciones = session
					.createQuery("select tage from TraduccionAgenda tage where tage.id.codigoAgenda=" + id).list();
			...
			if (traducciones != null) {
				...
				for (final TraduccionAgenda tra : traducciones) {
					...
					if (tra.getDocumento() != null) {
						...
						final SolrPendienteDelegate pendienteDel = DelegateUtil.getSolrPendienteDelegate();
						pendienteDel.grabarSolrPendiente(EnumCategoria.GUSITE_AGENDA.toString(), agenda.getId(),
								tra.getDocumento().getId(), IndexacionUtil.DESINDEXAR);
					}
	 ...
	 * @ejb.permission unchecked="true"
	 */
	private ArrayList<Agenda> crearlistadostateful(final List<?> listado, final String idioma) {
		final ArrayList<Agenda> lista = new ArrayList<Agenda>();
	...
	public boolean checkSite(final Long site, final Long id) {
		...
		try {
			final Query query = session.createQuery(
					"from Agenda age where age.idmicrosite=" + site.toString() + " and age.id=" + id.toString());
	 ...
	 *
	 */
	private Agenda obtenerAgendaBySolr(final Long id) {
		final Session session = this.getSession();
		try {
	...
	public SolrPendienteResultado indexarSolr(final SolrIndexer solrIndexer, final Long idElemento,
		...
		try {
			...
			final Microsite micro = micrositedel.obtenerMicrosite(agenda.getIdmicrosite());
			...
			if (agenda.getActividad() == null) {
				return new SolrPendienteResultado(false,
						"No se puede indexar, El evento no tiene asignada una actividad.");
			}
			...
			for (final String keyIdioma : agenda.getTraducciones().keySet()) {
				...
				if (traduccion != null && enumIdioma != null) {
					...
					descripcion.addIdioma(enumIdioma,
							traduccion.getDescripcion() != null ? solrIndexer.htmlToText(traduccion.getDescripcion())
									: "");

					final TraduccionActividadagenda tradActividad = (TraduccionActividadagenda) agenda.getActividad()
							.getTraduccion(keyIdioma);
					final String search = StringUtils.defaultString(traduccion.getTitulo()) + " "
	...
	public SolrPendienteResultado indexarSolrArchivo(final SolrIndexer solrIndexer, final Long idElemento,
		...
		log.debug("AgendafacadeEJB.indexarSolrArchivo. idElemento:" + idElemento + " categoria:" + categoria
				+ " idArchivo:" + idArchivo);
			...
			for (final String keyIdioma : agenda.getTraducciones().keySet()) {
				...
				if (traduccion != null && enumIdioma != null) {
					...
					if (traduccion.getDocumento() == null
							|| traduccion.getDocumento().getId().longValue() != idArchivo.longValue()) {
						continue;
					}
	 ...
	 * @ejb.permission unchecked="true"
	 */
	public List<Agenda> obtenerAgendasByMicrositeId(final Long idMicrosite) {

		final Session session = this.getSession();

		try {

			final Query query = session.createQuery("from Agenda a where a.idmicrosite =" + idMicrosite.toString());

			final List<Agenda> lista = query.list();
