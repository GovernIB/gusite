 ...
 * 
 * @ejb.bean name="sac/micropersistence/ConvocatoriaFacade"
...
public abstract class ConvocatoriaFacadeEJB extends HibernateEJB {
	...
	public void init(Long convocatoria, Long microsite) {
		...
		super.where = " where c.id = " + convocatoria
	...
	public Long grabarConvocatoria(Convocatoria convocatoria)
		...
		try {
			...
			this.grabarAuditoria(convocatoria, op);
	...
	public void ponerDestinatario(Long idConvocatoria, Long idLDistribucion)
		...
		try {
			Transaction tx = session.beginTransaction();
			Convocatoria conv = (Convocatoria) session.get(Convocatoria.class,
					idConvocatoria);
			Microsite microsite = conv.getMicrosite();
			DistribucionConvocatoria distrConv = new DistribucionConvocatoria(
					idConvocatoria, idLDistribucion);
			...
			this.grabarAuditoria(microsite.getId(), distrConv, Auditoria.CREAR);
	...
	public void quitarDestinatarios(Long idConvocatoria)
		...
		try {
			Transaction tx = session.beginTransaction();
			Criteria distribCrit = session
					.createCriteria(DistribucionConvocatoria.class);
			distribCrit.add(Restrictions.eq("idConvocatoria", idConvocatoria));
			...
			this.grabarAuditoria(conv, Auditoria.ELIMINAR);
	...
	public Convocatoria obtenerConvocatoria(Long id) {
		...
		Session session = this.getSession();
		try {
			Convocatoria convocatoria = (Convocatoria) session.get(
	...
	public boolean existeConvocatoria(Long id) {
		...
		try {
			List<?> lista = session.createQuery(
					"from Convocatoria c where c.id = " + id.toString()).list();
			return (lista.size() > 0);
	...
	public void borrarConvocatoria(Long id) throws DelegateException {
		...
		try {
			...
			this.grabarAuditoria(convocatoria, Auditoria.ELIMINAR);
	...
	public List<?> listarEncuestasConvocatoria() throws DelegateException {
		...
		try {
			this.parametrosCons(); // Establecemos los parámetros de la
									// paginación
			Query query = session
					.createQuery("SELECT distinct enc.*"
							...
							+ " and (pre_correo.multiresp = 'N' or"
							+ " (pre_correo.multiresp = 'S' and pre_correo.maxContestadas = 1 and pre_correo.minContestadas = 1))"
	...
	public List<?> listarCorreosEnvio(Long idConvocatoria, Integer nIntentos)
		...
		try {
			this.parametrosCons(); // Establecemos los parámetros de la
									// paginación
			Query query = session
					.createQuery("select distinct des.correo"
	...
	public List<?> listarCorreosEnvio(Long idConvocatoria, Integer nIntentos,
		...
		try {
			this.parametrosCons(); // Establecemos los parámetros de la
									// paginación
			Query query = session
					.createQuery("select distinct c.correo"
							+ " from Correo as c"
							+ " where c.noEnviar = 0"
							+ " and c.correo = :email"
