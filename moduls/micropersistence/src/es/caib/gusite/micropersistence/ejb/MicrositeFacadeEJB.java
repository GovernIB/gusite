...
public abstract class MicrositeFacadeEJB extends HibernateEJB {
    ...
    public Long grabarMicrosite(Microsite site) {
        ...
        try {
            ...
            if (nuevo) {
            	...
            	site.setClaveunica(obtenerClaveUnica(site));
            }
    ...
    public void borrarUsuarioPropietarioMicrosite(UsuarioPropietarioMicrosite upm) {
        ...
        try {
        		...
        		criteri.add(Expression.eq("idmicrosite", upm.getPk().getIdmicrosite()));
        		criteri.add(Expression.eq("idusuario", upm.getPk().getIdusuario()));
    ...
    public MicrositeCompleto obtenerMicrositeCompletobyKey(String key) {
        ...
        Session session = getSession();
        try {
            String hql = "select mic" +
                    " from MicrositeCompleto mic" +
...
//            return (list.size() > 0) ? (MicrositeCompleto) list.get(0) : null;
    ...
    public void borrarMicrositeCompleto(Long id) {
        ...
        try {
        	...
        	Criteria criteri = session.createCriteria(UsuarioPropietarioMicrosite.class);
            criteri.add(Expression.eq("pk.idmicrosite", id));
            ...
            if (!listIdRespu.isEmpty()) {
	            ...
	            criteriUsuPropiResp.add(Expression.in("id.idrespuesta", listIdRespu));
    ...
    private List<?> idRespDeEncDelMicrosite(Long idMicrosite) {
        ...
        try {
            String hql = "SELECT RESP.id " +
                    ...
                    "AND PRE.idencuesta = ENC.id " +
                    "AND ENC.idmicrosite = " + idMicrosite.toString();
    ...
    public Long reemplazarMicrositeCompleto(MicrositeCompleto site) {
        ...
        try {
            ...
            if (oldsite != null) {
                ...
                borrarMicrositeCompleto(oldsite.getId());
	            session.flush();
            }
            //Tercero: grabar el nuevo microsite
            grabarMicrositeCompleto(site);
            session.flush();
            tx.commit();
            close(session);
    ...
    public List<?> listarMicrositesbyUser(Usuario usuario) {
        ...
        try {
            ...
            hql += " where upm.pk.idusuario=" + usuario.getId().longValue() + " and mic.id = upm.pk.idmicrosite and trad.id.codigoIdioma = '" + Idioma.getIdiomaPorDefecto() + "' order by trad.titulo";
    ...
    public List<?> llistarMicrodeluser(String id) {
        ...
        try {
            String hql = "select mic.id, mic.unidadAdministrativa, trad.titulo ";
            hql += " from UsuarioPropietarioMicrosite upm, Microsite mic join mic.traducciones trad ";
            hql += " where upm.pk.idusuario = " + id + " and  mic.id = upm.pk.idmicrosite and trad.id.codigoIdioma = '" + Idioma.getIdiomaPorDefecto() + "' order by trad.titulo";
	...
	public ModelFilterObject obtenerFilterObject(Long idsite) {
		...
		try {
    		...
    		while (idPadre != null) {
        		...
        		UnitatAdministrativaQueryServiceAdapter padre = rqs.obtenirUnitatAdministrativa(uaCriteria);
			...
			for (int i = 0; i < langs.size(); i++) {
	    		...
	    		// Materias. se identifican por las que hay en las fichas a traves de su url  
