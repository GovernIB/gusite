...
public abstract class ContenidoFacadeEJB extends HibernateEJB {
    ...
    public void init(String menu) {
    	...
    	super.where = " where trad.id.codigoIdioma = '" + Idioma.getIdiomaPorDefecto() + "' and contenido.idmenu = " + menu;
    	super.whereini = " ";
    ...
    public Long grabarContenido(Contenido contenido) throws DelegateException {
         ...
         try {
             ...
             if (nuevo) {
                 Iterator<TraduccionContenido> it = contenido.getTraducciones().values().iterator();
                 while (it.hasNext()) {
    ...
    public boolean existeContenido(Long id) {
        ...
        Session session = getSession();
        try {
            List<?> lista = session.createQuery("from Contenido conte where conte.id = " + id.toString()).list();
            return (lista.size() > 0);
    ...
    public void borrarContenido(Long id) throws DelegateException {
        ...
        try {
        	...
        	session.createQuery("delete Archivo arch where arch.pagina = " + id.toString()).executeUpdate();
            ...
            if ((menu1.getTraduccion(Idioma.getIdiomaPorDefecto())) != null) {
   			...
   			if (idmenupadre.intValue() != 0) { // Era el hijo
       			...
       			if (menu2.getTraduccion(Idioma.getIdiomaPorDefecto()) != null) {
                    ret = ((TraduccionMenu) menu2.getTraduccion(Idioma.getIdiomaPorDefecto())).getNombre() + " > " + padre1 + " > " + contnombre;
                } else {
                    ret = " > " + padre1 + " > " + contnombre;
                }
   			}
    ...
    public List<?> listarAllContenidos(String micro) throws DelegateException {
    	 ...
    	 Session session = getSession();
         try {
             String sql = "select contenido from Contenido contenido where contenido.menu.microsite.id = " + micro;
	...
	public boolean checkSite(Long id, Long idsite) throws DelegateException  {
    	...
    	MenuDelegate bdMenu = DelegateUtil.getMenuDelegate();
    	Long idmicro = bdMenu.obtenerMenu(con.getMenu().getId()).getMicrosite().getId();
...
/***************************************************************************************/
	...
	public void indexInsertaContenido(Contenido con, ModelFilterObject filter) {
		...
		try {
		    if (filter == null) {
		        Long idmicro = DelegateUtil.getMenuDelegate().obtenerMenu(con.getMenu().getId()).getMicrosite().getId();
			...
			for (int i = 0; i < langs.size(); i++) {
                ...
                try {
                    ...
                    io.setTitulo("");
                    io.setUrl("/sacmicrofront/contenido.do?lang=" + idioma + "&idsite=" + filter.getMicrosite_id().toString() + "&cont=" + con.getId());
                    io.setCaducidad("");
                    io.setPublicacion("");
                    io.setDescripcion("");
                    ...
                    if (con.getFcaducidad() != null) {
                        io.setCaducidad(new java.text.SimpleDateFormat("yyyyMMdd").format(con.getFcaducidad()));
                    }

                    if (con.getFpublicacion() != null) {
                        io.setPublicacion(new java.text.SimpleDateFormat("yyyyMMdd").format(con.getFpublicacion()));
                    }

                    TraduccionContenido trad = ((TraduccionContenido) con.getTraduccion(idioma));
                    if (trad != null) {
