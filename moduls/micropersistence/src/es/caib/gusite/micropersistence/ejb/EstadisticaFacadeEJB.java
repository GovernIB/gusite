 ...
 * 
 * @ejb.bean name="sac/micropersistence/EstadisticaFacade"
...
public abstract class EstadisticaFacadeEJB extends HibernateEJB {
	...
	/**
	 * Inicializo los parámetros de la consulta de Estadistica.... No está bien
	...
	public void init() {
		...
		super.camposfiltro = new String[] { "stat.item", "stat.referencia",
				"stat.mes" };
		super.cursor = 0;
		super.nreg = 0;
		super.npags = 0;
	...
	public Estadistica obtenerEstadistica(Long id) {
		...
		Session session = this.getSession();
		try {
			Estadistica estadistica = (Estadistica) session.get(
					Estadistica.class, id);
			return estadistica;
	...
	public Estadistica obtenerEstadistica(Long idmicrosite, Long item,
			String ref, Integer mes, Integer publico) {
		...
		try {
			List<?> estadisticas = session.createCriteria(Estadistica.class)
					...
					.add(Restrictions.eq("publico", publico)).list();

			Estadistica estadistica = null;
			if (estadisticas.isEmpty()) {
				estadistica = new Estadistica(idmicrosite, item, ref,
						mes.intValue(), publico.intValue());
			} else {
	...
	public List<Estadistica> listarEstadisticas() {
		...
		try {
			...
			return this.crearlistadostateful(query.list());
	...
	public void borrarEstadisticasMicroSite(Long idmicrosite) {
		...
		Session session = this.getSession();
		try {
			List<?> estadisticas = session.createQuery(
					"from Estadistica as a where a.idmicrosite = "
							+ idmicrosite).list();
			Estadistica estadistica = null;
			if (!estadisticas.isEmpty()) {
				Iterator<?> iter = estadisticas.iterator();
	...
	/**
	 * Crea la Estadistica de un MicroSite que se sobreescribe estaba.
	 * 
	 * @ejb.interface-method
	 * @ejb.permission unchecked="true"
	 */
	public Long crearEstadisticasMicroReemplazado(Long antiguo, Long nuevo) {
		...
		try {
			...
			Query query = session.createQuery(this.select + this.from
					+ this.where + groupby + this.orderby);
			query.setFirstResult(this.cursor - 1);
