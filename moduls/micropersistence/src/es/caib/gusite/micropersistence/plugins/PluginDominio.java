...
public class PluginDominio {
	 ...
	 * @throws Exception
	 */
	public Map<?, ?> obtenerListado(Tipo dominio, Map<?, ?> parametros)
			throws Exception {

		// Accedemos a dominio
		Map<?, ?> valoresDominio = this.resuelveDominioEJB(dominio, parametros);
	 ...
	 * @throws Exception
	 */
	private Map<?, ?> resuelveDominioEJB(Tipo dominio, Map<?, ?> parametros)
			throws Exception {
		...
		try {
			if ((dominio.getXusr() != null) && (!dominio.getXusr().equals(""))) {
				...
				handler = new UsernamePasswordCallbackHandler(
						dominio.getXusr(), dominio.getXpwd());
				lc = new LoginContext("client-login", handler);
				lc.login();
			}

			home = (DominioFacadeHome) this.lookupHome(dominio, jndiName,
					DominioFacadeHome.class);
	...
	private Object lookupHome(Tipo dominio, String jndiName,
			Class<DominioFacadeHome> narrowTo) throws Exception {
		log.debug("Acceso remoto a " + dominio.getXjndi() + " ["
				+ dominio.getXurl() + "]");
		return UtilEjb.lookupHome(dominio.getXjndi(), false /* local */,
				dominio.getXurl(), narrowTo);
	}
