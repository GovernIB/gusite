 ...
 *
 * @ejb.bean
 *  name="sac/micropersistence/RespuestaDatoFacade"
 *  jndi-name="org.ibit.rol.sac.micropersistence.RespuestaDatoFacade"
 *  type="Stateless"
 *  view-type="remote"
...
public abstract class RespuestaDatoFacadeEJB extends HibernateEJB {
    ...
    public List<?> listarDatosByEnc(Long id) {
        Session session = getSession();
        try {
        	String hql=" from RespuestaDato resdat where resdat.idencuesta=" + id.toString();
    ...
    public List<?> listarDatosByPreg(Long id) {
        Session session = getSession();
        try {
        	String hql=" from RespuestaDato resdat where resdat.idpregunta=" + id.toString();
    ...
    public List<?> listarDatosByResp(Long id) {
        Session session = getSession();
        try {
        	String hql=" from RespuestaDato resdat where resdat.idrespueta=" + id.toString();
        	Query query = session.createQuery(hql);
        	return query.list();
    ...
    public ArrayList<RespuestaDato> listarDatosByResp2(Long id) {
        ...
        Session session = getSession();
        try {
        	String hql=" select resdat.dato, count(resdat.dato) from RespuestaDato resdat where resdat.idrespueta=" + id.toString() + " group by resdat.dato order by count(resdat.dato) desc";
        	...
        	while ( res.hasNext() ) {
                ...
                if (valor==null || valor.equals("null") || valor.length()==0) valor="[vacio]";
                resdat.setDato(valor);
                if (ncount!=null && !ncount.equals("null") && ncount.length()>0 && !ncount.equals("0")) {
                	resdat.setIdrespueta(new Long(ncount)); //se utiliza la propiedad idrespuesta para almacenar las repeticiones
