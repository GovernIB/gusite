...
/**
 * Procesador de plantillas thymeleaf, usado para visualizar componentes
 * embebidos.
 * 
 * @author agarcia
 * 
 */
@Component
public class GusiteTemplateProcessor {
	...
	private static Log log = LogFactory.getLog(GusiteTemplateProcessor.class);
	...
	public String process(Map<String, Object> model, final String templateName, String lang, HttpServletRequest request, HttpServletResponse response) {
		...
		IWebContext ctx = new WebContext(request, response, this.servletContext, locale, model);
		...
		try {
			if (templateName.contains("::")) {
				...
				return this.templateEngine.process(templateFile, ctx, fragmentSpec);
