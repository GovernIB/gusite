...
public abstract class BaseController extends FrontController {
	...
	public Microsite loadMicrosite(String uri, Idioma lang, Model model)
	 ...
	 */
	public Microsite loadMicrosite(String uri, Idioma lang, Model model,
			String pcampa) throws ExceptionFrontMicro {

		/* El idioma ya viene fijado en la URI */
		String idi = lang.getLang().toUpperCase();
		model.addAttribute("MVS_idioma", idi);

		Microsite microsite = this.cargarSite(uri, lang, model);

		// TODO: comprobar que el idioma es de los válidos para el microsite. En
		// caso contrario habria que lanzar un 404 not found
		this.cargarListaIdiomas(microsite, model);

		// cargarUA(microsite, model, lang);
		this.cargarCss(microsite, model);
		this.cargarPie(microsite, model, lang);
		this.cargarCabecera(microsite, model, lang);
		this.cargarMenu(microsite, model, lang);
	...
	private Microsite cargarSite(String uri, Idioma lang, Model model)
			throws ExceptionFrontMicro {

		Microsite microsite = null;
		try {
			DelegateBase _delegateBase = new DelegateBase();
			microsite = _delegateBase
					.obtenerMicrositebyUri(uri, lang.getLang());

			if (microsite == null) {
				throw new ExceptionFrontMicro(
						" [Configuracion microsite]: Se debe indicar algún microsite");
			}

			if (!"S".equals(microsite.getVisible())) {
				throw new ExceptionFrontMicro(
						" [Configuracion microsite]: El site no es visible");
			}
		...
		String titulo_mic = ((TraduccionMicrosite) microsite.getTraduccion(lang
				.getLang()) != null) ? ((TraduccionMicrosite) microsite
				.getTraduccion(lang.getLang())).getTitulo() : "&nbsp;";
		model.addAttribute("MVS_micrositetitulo", titulo_mic);

		String descripcion_mic = ((TraduccionMicrosite) microsite
				.getTraduccion(lang.getLang()) != null) ? ((TraduccionMicrosite) microsite
				.getTraduccion(lang.getLang())).getDescription() : "&nbsp;";
		model.addAttribute("MVS_micrositedescription", descripcion_mic);

		String keywords_mic = ((TraduccionMicrosite) microsite
				.getTraduccion(lang.getLang()) != null) ? ((TraduccionMicrosite) microsite
				.getTraduccion(lang.getLang())).getKeywords() : "&nbsp;";
		model.addAttribute("MVS_micrositekeywords", keywords_mic);

		String analytics_mic = ((TraduccionMicrosite) microsite
				.getTraduccion(lang.getLang()) != null) ? microsite
				.getAnalytics() : "&nbsp;";
	 ...
	 * @throws Exception
	 */
	private void cargarListaIdiomas(Microsite microsite, Model model)
			throws ExceptionFrontMicro {
		...
		try {
			...
			while (iter.hasNext()) {
				String tmpidioma = ((IdiomaMicrosite) iter.next()).getId()
						.getCodigoIdioma().toLowerCase();
				String tmptxtidioma = "";
			...
			while (iteridi.hasNext()) {
				...
				if (hashidioma.containsKey(tmpidioma)) {
					listaidiomas.add(new Pardato(tmpidioma, (String) hashidioma
							.get(tmpidioma)));
				}
			}
			...
			throw new ExceptionFrontMicro(
					" [Configuracion microsite] Preparando idiomas...: "
							+ e.getMessage());
		}
	}
	 ...
	 * @throws Exception
	 */
	private void cargarCss(Microsite microsite, Model model)
			throws ExceptionFrontMicro {
		// MCR v1.1
		MParserHTML parserhtml = new MParserHTML();
		if (microsite.getEstiloCSS() != null) {
			model.addAttribute("MVS_css", parserhtml.tagCSS(microsite.getId(),
					microsite.getEstiloCSS().getId(),
					microsite.getEstiloCSSPatron()));
		} else {
			model.addAttribute(
					"MVS_css",
					parserhtml.tagCSS(null, null,
							microsite.getEstiloCSSPatron()));
		}
	}
	 ...
	 * @param request
	 */
	private void cargarPie(Microsite microsite, Model model, Idioma lang)
			throws ExceptionFrontMicro {

		try {

			/* sergio_ini */
			this.direccionPie(new Long(microsite.getUnidadAdministrativa()),
					microsite, model, lang);
			/* sergio_fin */
			...
			while (iter.hasNext()) {
				...
				tmpidioma = ((IdiomaMicrosite) iter.next()).getId()
						.getCodigoIdioma().toLowerCase();

				micrositetra = (TraduccionMicrosite) microsite
						.getTraduccion(tmpidioma);
				if (micrositetra != null) {
					if ((micrositetra.getTxtop3() != null)
							&& (micrositetra.getUrlop3() != null)) {
						par = new Pardato(micrositetra.getTxtop3(),
								micrositetra.getUrlop3());
						if (microsite.getOpt3().equals("S")) {
							lista.add(par);
						}
					}
					if ((micrositetra.getTxtop4() != null)
							&& (micrositetra.getUrlop4() != null)) {
						par = new Pardato(micrositetra.getTxtop4(),
								micrositetra.getUrlop4());
						if (microsite.getOpt4().equals("S")) {
							lista.add(par);
						}
					}
					if ((micrositetra.getTxtop5() != null)
							&& (micrositetra.getUrlop5() != null)) {
						par = new Pardato(micrositetra.getTxtop5(),
								micrositetra.getUrlop5());
						if (microsite.getOpt5().equals("S")) {
							lista.add(par);
						}
					}
					if ((micrositetra.getTxtop6() != null)
							&& (micrositetra.getUrlop6() != null)) {
						par = new Pardato(micrositetra.getTxtop6(),
								micrositetra.getUrlop6());
						if (microsite.getOpt6().equals("S")) {
							lista.add(par);
						}
					}
					if ((micrositetra.getTxtop7() != null)
							&& (micrositetra.getUrlop7() != null)) {
						par = new Pardato(micrositetra.getTxtop7(),
								micrositetra.getUrlop7());
	...
	protected void direccionPie(Long coduo, Microsite microsite, Model model,
			Idioma lang) throws QueryServiceException {
		...
		List<EdificiQueryServiceAdapter> edificios = ua
				.llistarEdificis(edificiCriteria);

		String idUOPresidencia = System
				.getProperty("es.caib.gusite.codigoUO.presidencia");
		if (idUOPresidencia == null) {
			throw new RuntimeException(
					"No se estableció la propiedad de sistema es.caib.gusite.codigoUO.presidencia");
		}
		...
		if (edificios.size() > 0) {
			...
			if (UO_PRESIDENCIA.compareTo(coduo) == 0) {
				direccion
						.append("<a href=\"" + absUrl
								+ "/govern/presidencia.do?lang=")
						.append(lang.getLang()).append("\">")
						.append(ua.getNombre());

			} else {
				direccion
						.append("<a href=\"" + absUrl
								+ "/govern/organigrama/area.do?coduo=")
						.append(coduo).append("&lang=").append(lang.getLang())
						.append("\">").append(ua.getNombre());
			}

			direccion.append("</a>");
			direccion
					.append("<a href=\"" + absUrl
							+ "/govern/organigrama/planol.do?coduo=")
					.append(coduo).append("&lang=").append(lang.getLang())
					.append("\">");
			direccion.append(": ").append(edificio.getDireccion())
					.append(" - ").append(edificio.getCodigoPostal())
					.append(" ").append(edificio.getPoblacion())
					.append("</a><br />");
			...
			if (ua.getTelefono() != null && ua.getTelefono().length() > 0) {
				tel = ua.getTelefono();
			} else if (edificio.getTelefono() != null
					&& edificio.getTelefono().length() > 0) {
				tel = edificio.getTelefono();
			}

			if (tel != null && tel.length() > 0) {
				direccion.append(this.getMessage("WEB_ILL067", lang))
						.append(" ").append(tel);
			}

			if (ua.getFax() != null && ua.getFax().length() > 0) {
				fax = ua.getFax();
			} else if (edificio.getFax() != null
					&& edificio.getFax().length() > 0) {
				fax = edificio.getFax();
			}

			if (tel != null && tel.length() > 0 && fax != null
					&& fax.length() > 0) {
				direccion.append(" - ");
			}

			if (fax != null && fax.length() > 0) {
				direccion.append(this.getMessage("WEB_ILL068", lang))
						.append(" ").append(fax);
			}

			if (microsite.getDomini() != null
					&& microsite.getDomini().length() > 0) {
				direccion.append("<br />")
						.append(this.getMessage("WEB_ILL130", lang))
						.append(" <a href=").append(microsite.getDomini())
						.append(">").append(microsite.getDomini())
						.append("</a>");
			} else {
				if (ua.getUrl() != null && ua.getUrl().length() > 0) {
					direccion.append("<br />")
							.append(this.getMessage("WEB_ILL130", lang))
							.append(" <a href=").append(ua.getUrl())
							.append(">").append(ua.getUrl()).append("</a>");
				}
			}
	 ...
	 * @param request
	 */
	private void cargarCabecera(Microsite microsite, Model model, Idioma lang)
			throws ExceptionFrontMicro {
		Iterator<IdiomaMicrosite> iter = microsite.getIdiomas().iterator();
		TraduccionMicrosite micrositetra;
		String tmpidioma = "";
		...
		while (iter.hasNext()) {
			...
			if (microsite.getOptMapa().equals("S")) {
				trio = new Tridato(this.getMessage("micro.es.mapa", lang),
						urlFactory.mapa(microsite, lang), "mapaWeb");
				lista.add(trio);
			}
			if (microsite.getOptContacto().equals("S")) {
				trio = new Tridato(this.getMessage("micro.es.contacto", lang),
						urlFactory.listarContactos(microsite, lang), "atencio");
				lista.add(trio);
			}
			if (microsite.getOptFaq().equals("S")) {
				trio = new Tridato(this.getMessage("micro.es.faq", lang),
						urlFactory.listarFaqs(microsite, lang));
				lista.add(trio);
			}
			if (micrositetra != null) {
				if ((micrositetra.getTxtop1() != null)
						&& (micrositetra.getUrlop1() != null)) {
					trio = new Tridato(micrositetra.getTxtop1(),
							micrositetra.getUrlop1());
					if (microsite.getOpt1().equals("S")) {
						lista.add(trio);
					}
				}
				if ((micrositetra.getTxtop2() != null)
						&& (micrositetra.getUrlop2() != null)) {
					trio = new Tridato(micrositetra.getTxtop2(),
							micrositetra.getUrlop2());
	 ...
	 * @param request
	 */
	private void cargarMenu(Microsite microsite, Model model, Idioma lang)
			throws ExceptionFrontMicro {
		...
		try {
			...
			model.addAttribute(
					"MVS_menu",
					delegateBase.obtenerMainMenu(microsite.getId(),
							lang.getLang()));
		} catch (DelegateException e) {
			throw new ExceptionFrontMicro(e);
		}
	...
	private void cargarServicio(Microsite microsite, Model model)
			throws ExceptionFrontMicro {
		model.addAttribute("MVS_servicio", this.setServicio());
	}
	...
	private void cargarCampanya(Microsite microsite, Model model, Idioma lang,
		...
		if (pcampa != null && pcampa.equals("yes")) {
			MParserHTML parserhtml = new MParserHTML();
			String tagHtmlTmpCampaya = parserhtml.getHtmlCampanya(microsite,
					lang.getLang()).toString();
			if (tagHtmlTmpCampaya.length() > 1) {
				model.addAttribute("MVS_home_tmp_campanya", tagHtmlTmpCampaya);
			}
		}
	}
	 ...
	 *         refservicio
	 */
	protected boolean existeServicio(Microsite microsite, Idioma lang,
			String refservicio) {

		boolean tmp = false;
		try {
			DelegateBase _delegateBase = new DelegateBase();
			ArrayList<?> listserofr = _delegateBase
					.obtenerListadoServiciosMicrosite(microsite, lang.getLang());
	 ...
	 * @return
	 */
	public List<PathItem> getBasePath(Microsite microsite, Model model,
			Idioma lang) {
		...
		path.add(new PathItem(rb.getString("general.inicio"),
				"http://www.caib.es"));

		UnitatAdministrativaDTO ua = this.dataService.getUa(
				microsite.getUnidadAdministrativa(), lang);
		...
		while (ua != null) {
			...
			if (ua.getPadre() != null && ua.getPadre() != 1) { // TODO: id UA
																// Govern
																// hardcoded

				ua = this.dataService.getUa(ua.getPadre(), lang);
