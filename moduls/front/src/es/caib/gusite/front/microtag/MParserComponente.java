...
public class MParserComponente extends MParserHTML {
	 ...
	 *         listado
	 */
	public StringBuffer getHtmlElementosComponente(Long idmicrosite,
			String idcomponente, String idioma) {
		StringBuffer retorno = new StringBuffer();

		try {
			ComponenteDelegate compodel = DelegateUtil.getComponentesDelegate();
			Componente componente = compodel.obtenerComponente(new Long(Long
					.parseLong(idcomponente)));

			// si es de clase noticia.
			if (componente.getTipo().getTipoelemento()
					.equals(Microfront.ELEM_NOTICIA)) {
				retorno.append(this.getHtmlComponenteTNoticia3(idmicrosite,
						componente, idioma));
			}
			// si es de clase enlace.
			if (componente.getTipo().getTipoelemento()
					.equals(Microfront.ELEM_LINK)) {
				retorno.append(this.getHtmlComponenteTEnlace(idmicrosite,
						componente, idioma));
			}
			// si es de clase documento
			if (componente.getTipo().getTipoelemento()
					.equals(Microfront.ELEM_DOCUMENTO)) {
				retorno.append(this.getHtmlComponenteTDocumento(idmicrosite,
						componente, idioma));
			}
			// si es de clase documento
			if (componente.getTipo().getTipoelemento()
					.equals(Microfront.ELEM_CONEXIO_EXTERNA)) {
				retorno.append(this.getHtmlComponenteTExterno(idmicrosite,
						componente, idioma));
			}
	 ...
	 */
	private StringBuffer getHtmlComponenteTExterno(Long idmicrosite,
			Componente componente, String idioma) {
		StringBuffer retorno = new StringBuffer();

		try {
			String nombreformulario = "formnoticiasexternocomp"
					+ componente.getId().longValue();
			String nombre_elemento = ((TraduccionComponente) componente
					.getTraduccion(idioma)) != null ? ((TraduccionComponente) componente
			...
			retorno.append("<div id=\"element").append(componente.getId())
					.append("\">");
			retorno.append("<h2>").append(nombre_elemento).append("</h2>");
			retorno.append("<form action=\"noticias.do\" name=\"")
					.append(nombreformulario).append("\" id=\"")
					.append(nombreformulario).append("\" >");

			retorno.append("<input type=\"hidden\" name=\"lang\" value=\"")
					.append(idioma).append("\">");
			retorno.append("<input type=\"hidden\" name=\"nameform\" value=\"")
					.append(nombreformulario).append("\">");
			...
			String resultadoHTML = tD.obtenerPegoteHTMLExterno(componente
					.getTipo().getId(), filtrado);
	 ...
	 * @return StringBuffer Código Html
	 */
	private StringBuffer getHtmlComponenteTNoticia3(Long idmicrosite,
			Componente componente, String idioma) {
		StringBuffer retorno = new StringBuffer();
		...
		try {
			...
			noticiadel.setOrderby2(this.preparaOrden(componente.getOrdenacion()
					.toString()));
			...
			if (listanoticias.size() != 0) {
				...
				String nCols = (componente.getSoloimagen().equals("S")) ? ""
						: "<tr>";
				while (iter.hasNext()) {
					java.text.SimpleDateFormat dia = new java.text.SimpleDateFormat(
							"dd/MM/yyyy");
					Noticia noti = (Noticia) iter.next();

					retorno.append(((cont % 2) == 0) ? "<tr class=\"par\">"
							: nCols);
					if (componente.getSoloimagen().equals("S")) {
						// en el caso de que sólo imágenes
						retorno.append("<td style=\"width:1%\" >");
						retorno.append("<a href=\""
								+ MicroURI.uriNoticia(idmicrosite,
										noti.getId(), idioma) + "\">");
						if (noti.getImagen() != null) {
							retorno.append("<img src=\""
									+ MicroURI.uriImg(Microfront.RNOTICIA, noti
											.getId().longValue(), noti
											.getImagen().getId().longValue())
									+ "\" alt=\"\" class=\"imagen\" />");
						} else {
							if (componente.getImagenbul() != null) {
								retorno.append("<img src=\""
										+ MicroURI.uriImg(
												Microfront.RMICROSITE,
												idmicrosite.longValue(),
												componente.getImagenbul()
														.getId().longValue())
										+ "\" alt=\""
										+ ((TraduccionNoticia) noti
							...
							retorno.append(((TraduccionNoticia) noti
						...
						retorno.append(noti.getFpublicacion() != null ? dia
								.format(noti.getFpublicacion()) : "");
						retorno.append("&nbsp;<a href=\""
								+ MicroURI.uriNoticia(idmicrosite,
										noti.getId(), idioma) + "\">");
						retorno.append(((TraduccionNoticia) noti
								.getTraduccion(idioma)).getTitulo() + "</a>");
						String subTitol = ((TraduccionNoticia) noti
								.getTraduccion(idioma)).getSubtitulo();
						retorno.append("<br/>"
								+ (subTitol == null ? "" : subTitol) + "<br/>");
				...
				retorno.append("<p>" + rb.getString("microhtml.nonoticias")
						+ "</p>");
			}
			retorno.append("</div>");
			if (idmicrosite.longValue() == 0) {
	 ...
	 * @return StringBuffer Código HTML
	 */
	private StringBuffer getHtmlComponenteTEnlace(Long idmicrosite,
			Componente componente, String idioma) {
		StringBuffer retorno = new StringBuffer();
			...
			if (listanoticias.size() != 0) {
				...
				while (iter.hasNext()) {
					...
					if (componente.getSoloimagen().equals("S")
						...
						if (componente.getImagenbul() != null) {
							retorno.append("<img src=\""
									+ MicroURI.uriImg(Microfront.RMICROSITE,
											idmicrosite.longValue(), componente
													.getImagenbul().getId()
													.longValue())
									+ "\" alt=\"\" class=\"imagen\" />");
						} else {
						...
						boolean urlExt = ((TraduccionNoticia) noti
								.getTraduccion(idioma)).getLaurl().startsWith(
								"http:");
						String target = (urlExt) ? "_blank" : "_self";
						retorno.append("<a href=\""
								+ ((TraduccionNoticia) noti
										.getTraduccion(idioma)).getLaurl()
								+ "\" target=\""
								+ target
								+ "\">&nbsp;"
								+ ((TraduccionNoticia) noti
						...
						if (((TraduccionNoticia) noti.getTraduccion(idioma))
								.getSubtitulo() != null) {
							retorno.append("<br/>"
									+ ((TraduccionNoticia) noti
											.getTraduccion(idioma))
	 ...
	 * @return StringBuffer Código HTML
	 */
	private StringBuffer getHtmlComponenteTDocumento(Long idmicrosite,
			Componente componente, String idioma) {
		StringBuffer retorno = new StringBuffer();
		...
		try {
			...
			noticiadel.setTampagina(componente.getNumelementos().intValue());
			java.sql.Date dt = new java.sql.Date((new Date()).getTime());
			String wherenoticias = "where trad.id.codigoIdioma='" + idioma
					+ "' and noti.visible='S' and noti.idmicrosite="
					+ idmicrosite + " and noti.tipo="
					+ componente.getTipo().getId().toString();
			wherenoticias += " and (noti.fpublicacion is null OR to_char(noti.fpublicacion,'yyyy-MM-dd')<='"
					+ dt + "')";
			wherenoticias += " and (noti.fcaducidad is null OR to_char(noti.fcaducidad,'yyyy-MM-dd')>='"
			...
			if (listanoticias.size() != 0) {
				...
				while (iter.hasNext()) {
					...
					if (componente.getSoloimagen().equals("S")
						...
						retorno.append("<a href=\"elementodocumento.do?idsite="
								+ idmicrosite
								+ "&cont="
								+ noti.getId()
								+ "&lang="
