		...
		try {
			...
			if ((encuesta.getVisible().equals("S"))
				...
				while (iter.hasNext()) { // Preguntas de una encuesta
					...
					if ((pregunta.getVisible().equals("S"))
						...
						if (pregunta.getImagen() != null) {
							retorno.append("<p><img src=\"archivopub.do?ctrl=MCRST"
									+ idmicrosite
									+ "ZI"
									+ pregunta.getImagen().getId()
									+ "&id="
									+ pregunta.getImagen().getId()
									+ "\" alt=\"\" /></p>\n");
						}
				...
				if (encuesta.getMostrar().equalsIgnoreCase("S")) {
					retorno.append("<p style=\"text-align:center;\"><a href=\"envioencuesta.do?"
							+ Microfront.PIDSITE
							+ "="
							+ idmicrosite
							+ "&"
							...
							+ Microfront.PVIEW
							+ "=yes\">"
							+ rb.getString("encuesta.verresultados")
							+ "</a></p>\n");
				}
				...
				retorno.append("<h3>"
						+ ((encuesta.getTraduce() != null) ? ((TraduccionEncuesta) encuesta
								.getTraduce()).getTitulo() : "&nbsp;")
						+ "</h3>\n");
				retorno.append("<p style=\"text-align:center;\">"
						+ rb.getString("encuesta.caducada") + "</p>\n");
				retorno.append("<p style=\"text-align:center;\"><a href=\"envioencuesta.do?"
						+ Microfront.PIDSITE
						+ "="
						+ idmicrosite
