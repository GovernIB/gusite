...
public class ContactosDataService {
	...
	public ResultadoBusqueda<Contacto> getListadoFormularios(
			Microsite microsite, Idioma lang, BaseCriteria formulario)
			throws DelegateException {

		ContactoDelegate contactodel = DelegateUtil.getContactoDelegate();
		contactodel.init();
		contactodel
				.setWhere("where contacto.visible='S' and contacto.idmicrosite="
						+ microsite.getId());

		if (formulario.getFiltro() != null
				&& formulario.getFiltro().length() > 0) {
			contactodel.setFiltro(formulario.getFiltro());
		}

		if (formulario.getOrdenacion() != null
				&& formulario.getOrdenacion().length() > 0) {
		...
		List<Contacto> listacontactos = (List<Contacto>) contactodel
				.listarContactos();
		ResultadoBusqueda<Contacto> ret = new ResultadoBusqueda<Contacto>(
				listacontactos,
				(Map<String, Integer>) contactodel.getParametros());
		return ret;
