...
/**
 * Esta clase contiene m�todos que parsea los tags especiales de los microsites.
 * Devuelven trozos de c�digo HTML pertenecientes a la agenda
 * 
 * @author Indra
 *
 */
public class MParserAgenda extends MParserHTML {
	...
	public StringBuffer getHtmlAgendaListado(Long idmicrosite, String idioma, int numeventos) {
		...
		try {
	    	...
	    	agendadel.setWhere("where index(trad)='" + idioma + "' and agenda.visible='S' and agenda.idmicrosite=" + idmicrosite + " and to_char(agenda.finicio,'yyyy-MM-dd')>='" + dt + "'");
	...
	public StringBuffer getHtmlAgendaCalendario(Long idmicrosite, String idioma, int nummeses) {
		...
		try {
	    	AgendaDelegate agendadel = DelegateUtil.getAgendaDelegate();
	    	agendadel.init();
	    	agendadel.setWhere("where index(trad)='" + idioma + "' and agenda.visible='S' and agenda.idmicrosite=" + idmicrosite);
			...
			// Establecemos el valor al d�a 1
