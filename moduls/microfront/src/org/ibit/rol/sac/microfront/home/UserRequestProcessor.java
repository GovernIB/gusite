package org.ibit.rol.sac.microfront.home;
...
import org.ibit.rol.sac.microfront.home.actions.InvalidRolAction;
...
public class UserRequestProcessor extends RequestProcessor {
	 ...
	 * Si es distinto de "nobody" quiere decir que est� logado. Salimos y seguimos...
	 * Si es "nobody":
	...
	public boolean processPreprocess( HttpServletRequest req, HttpServletResponse resp ) {
		...
		//Si llegamos aqu�, redirigir a la pantalla de intranetlogin (que a su vez sera interceptada por SEYCON)
		try {
		...
		if (req != null) {
			...
			for (Object o : mapParameter.keySet()) {
				if (((String) o).startsWith("resp")) {
					...
					try {
						byte[] p = ((String[]) mapParameter.get(o))[0].getBytes("ISO-8859-1");
