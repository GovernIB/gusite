package es.caib.gusite.microfront.util.correo;
...
public class CorreoEngineService implements CorreoEngine {
	...
	protected static Log log = LogFactory.getLog(CorreoEngineService.class);
	  ...
	  private String mail_charset = "utf-8";
	  private boolean mail_isHTMLFormat=false;
	...
	public void initCorreo(String destinatario, String subject, boolean isHTMLFormat, StringBuffer body)  {
	  ...
	  /**
		* Comprueba que la propiedad de sistema es.caib.gusite.email.debugMode sea igual a [S|s].
		* 
		* @return
		*/
	  ...
	  public boolean enviarCorreo() {
		...
		if (isDebugMode()) {
		    ...
		    for (int i = 0; i < listadestinatarios.size(); i++) {
		      if (!send(listadestinatarios.get(i))) {
		    	  log.error("Error enviando correo a destinatario: " + listadestinatarios.get(i) );
		    	  tmpok=false;
		      }
		    }
		}
